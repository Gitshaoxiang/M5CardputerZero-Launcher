...
static uint8_t gree_block_checksum(const uint8_t *block, const uint16_t length)
{
  uint8_t sum = 10;
...
void IRGreeAC::checksum(const uint16_t length) {
  ...
  _.Sum = gree_block_checksum(_.remote_state, length);
}
...
bool IRGreeAC::validChecksum(const uint8_t state[], const uint16_t length) {
  ...
  return GETBITS8(state[length - 1], kHighNibble, kNibbleSize) ==
      gree_block_checksum(state, length);
}
