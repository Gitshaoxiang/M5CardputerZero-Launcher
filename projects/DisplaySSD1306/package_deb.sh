...
PACKAGE_NAME="${PACKAGE_NAME:-displayssd1306}"
PACKAGE_VERSION="${PACKAGE_VERSION:-0.1.0}"
PACKAGE_SUFFIX="${PACKAGE_SUFFIX:-m5stack1}"
DEB_ARCH="${DEB_ARCH:-arm64}"
MAINTAINER="${MAINTAINER:-M5Stack}"

BIN_NAME="M5CardputerZero-DisplaySSD1306"
BIN_SRC="${ROOT_DIR}/dist/${BIN_NAME}"
DESKTOP_SRC="${ROOT_DIR}/assets/applications/displayssd1306.desktop"
...
if [[ ! -x "${BIN_SRC}" ]]; then
    ...
    echo "Run: cd ${ROOT_DIR} && CardputerZero=y scons -j\$(nproc)" >&2
    exit 1
fi
...
install -Dm644 "${DESKTOP_SRC}" "${STAGE_DIR}/usr/share/APPLaunch/applications/displayssd1306.desktop"
...
Description: SSD1306 display demo for M5CardputerZero APPLaunch
 SSD1306 Grove OLED demo app with 128x64 carousel content.
EOF
