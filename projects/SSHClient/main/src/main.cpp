...
static char apply_ctrl_modifier(char ch)
{
...
static void raw_key_to_utf8(uint32_t code, char *out, size_t out_len)
{
    ...
    {
        ch = g_shift_down ? shifted_digit(code)
                          : (code == KEY_0 ? '0' : static_cast<char>('1' + (code - KEY_1)));
    }
    else if (ch == 0)
    {
        switch (code)
...
static void open_raw_keyboard()
{
    ...
    bool grab = env_enabled("SSHCLIENT_KEYBOARD_GRAB", true);
    if (grab)
    {
        ioctl(g_key_fd, EVIOCGRAB, 1);
    }
}
