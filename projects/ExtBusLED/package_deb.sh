...
PACKAGE_NAME="${PACKAGE_NAME:-extbusled}"
PACKAGE_VERSION="${PACKAGE_VERSION:-0.1.0}"
PACKAGE_SUFFIX="${PACKAGE_SUFFIX:-m5stack1}"
DEB_ARCH="${DEB_ARCH:-arm64}"
MAINTAINER="${MAINTAINER:-M5Stack}"

BIN_NAME="M5CardputerZero-ExtBusLED"
BIN_SRC="${ROOT_DIR}/dist/${BIN_NAME}"
DESKTOP_SRC="${ROOT_DIR}/assets/applications/extbusled.desktop"
...
install -Dm644 "${DESKTOP_SRC}" "${STAGE_DIR}/usr/share/APPLaunch/applications/extbusled.desktop"
...
Description: ExtBus LED test demo for M5CardputerZero APPLaunch
 Toggle a GPIO output as LED and read one GPIO input as button.
EOF
