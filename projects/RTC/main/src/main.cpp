...
static int input_device_score(const std::string &path, char *name, size_t name_len)
{
    ...
    if (ioctl(fd, EVIOCGBIT(0, sizeof(ev_bits)), ev_bits) >= 0 &&
        test_bit(EV_KEY, ev_bits, sizeof(ev_bits) / sizeof(ev_bits[0])))
    {
        score += 20;
    }
    ...
    if (ioctl(fd, EVIOCGBIT(EV_KEY, sizeof(key_bits)), key_bits) >= 0)
    {
        ...
        if (test_bit(KEY_4, key_bits, sizeof(key_bits) / sizeof(key_bits[0]))) score += 10;
        if (test_bit(KEY_8, key_bits, sizeof(key_bits) / sizeof(key_bits[0]))) score += 10;
    }
...
static void process_keyboard_events()
{
    ...
    while (handled < 32)
    {
        ...
        if (elm->key_state != KBD_KEY_RELEASED &&
            (elm->key_code == KEY_HOME || elm->key_code == KEY_ESC))
        {
            app_request_quit();
        }
        else
        {
            ui_handle_key_item(elm->key_code, elm->utf8[0] ? elm->utf8 : nullptr, elm->key_state);
        }
...
static void lv_linux_indev_init(void)
{
    ...
    pthread_t thread_id;
    if (pthread_create(&thread_id, nullptr, keyboard_read_thread, const_cast<char *>(g_keyboard_device.c_str())) == 0)
    {
